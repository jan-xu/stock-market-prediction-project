...
class LSTMModel(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=128, output_size=1, pred_horizon=1, recurrent_pred_horizon=False):
        ...
        self.linear = nn.Linear(hidden_layer_size, output_size * (pred_horizon if not recurrent_pred_horizon else 1))
    ...
    def _assert_pred_horizon(self):
        if self.pred_horizon > 1 and self.recurrent_pred_horizon:
            assert self.input_size == self.output_size, f"For recurrent prediction horizon, `input_size = {self.input_size}` and `output_size = {self.output_size}` must be equal."
    ...
    def forward(self, input_seq, reset_hidden=True):
        ...
        lstm_out_last = lstm_out[:, -1:] # Hidden layer of last time step, same as self.hidden_cell[0][:1].transpose(0,1)
        ...
        if self.pred_horizon > 1: # If we want to predict multiple timesteps
            if self.recurrent_pred_horizon:
                predictions = self.recurrent_step_forward(predictions, self.pred_horizon)
            else:
                predictions = predictions.reshape(-1, self.pred_horizon, self.output_size)
...
class CNNLSTMModel(LSTMModel):
    ...
    """
    def __init__(self, input_size=1, hidden_layer_size=128, output_size=1, pred_horizon=1, conv_kernel_size=5, conv_channels=16, recurrent_pred_horizon=False):
        # Note: input_size will be times 2 due to conv features
        super(CNNLSTMModel, self).__init__(input_size=2*input_size, hidden_layer_size=hidden_layer_size, output_size=output_size, pred_horizon=pred_horizon, recurrent_pred_horizon=recurrent_pred_horizon)
    ...
    def _assert_pred_horizon(self):
        if self.pred_horizon > 1 and self.recurrent_pred_horizon:
            assert self.input_size == 2*self.output_size, f"For recurrent prediction horizon, `input_size = {self.input_size}` and 2 * `output_size = {self.output_size}` must be equal."
    ...
    def forward_one_step(self, input):
        ...
        if self.recurrent_pred_horizon:
            ...
            input_aug = torch.cat([input[:, self.conv_kernel_size - 1:], conv_feat], dim=2)
