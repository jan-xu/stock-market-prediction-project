...
def _test_lstm_models(input_size, batch_size, time_steps, pred_horizon):
    ...
    model = LSTMModel(
        input_size=input_size, pred_horizon=pred_horizon, recurrent_pred_horizon=False
    )
    input_seq = torch.randn(batch_size, time_steps, input_size)
    output = model(input_seq)
...
def test_lstm_forward_one():
    ...
    model = CNNLSTMModel(
        input_size=INPUT_SIZE, conv_kernel_size=CNN_LSTM_CONV_KERNEL_SIZE
    )
...
def test_lstm_pred_horizon_recurrency():
    ...
    model = CNNLSTMModel(
        input_size=INPUT_SIZE,
        output_size=INPUT_SIZE,
        conv_kernel_size=CNN_LSTM_CONV_KERNEL_SIZE,
        pred_horizon=PRED_HORIZON,
        recurrent_pred_horizon=True,
    )
    input_seq = torch.randn(BATCH_SIZE, TIME_STEPS, INPUT_SIZE)
